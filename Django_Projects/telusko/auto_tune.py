...
def Performance_Optimzer(trial):
    ...
    for loop in range(2):
        ...
        _,_,_ = ssh.exec_command("cd rtejac/squeezenet1.1/;./run_multi.sh")
        
        _,stdout,stderr = ssh.exec_command("cd rtejac/squeezenet1.1/;source /opt/intel/openvino/bin/setupvars.sh;python3 /opt/intel/openvino/deployment_tools/tools/benchmark_tool/benchmark_app.py -m  squeezenet1.1.xml -i car.png -progress true  -t 10")
        lines = stdout.readlines()
            #performance = 0#random.randint(600,1000)#0
        for line in lines:
